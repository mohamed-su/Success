...
const DistributeProtocols: React.FC = () => {
  ...
  const handleSelectProtocol = (id: number) => {
    ...
    if (protocol?.isAssigned) {
      setMessage(`Le protocole PROT-${id} est déjà assigné à ${protocol.assignedTo}`);
      return;
    }
  ...
  const handleBulkAssign = async () => {
    ...
    try {
      const response = await fetch('${BASE_URL}/auto-assignment/bulk-assign', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold' }}>Distribuer les Protocoles</h1>
                ...
                <div key={protocol.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '14px', color: '#6b7280' }}>
                        <p>Chercheur: {protocol.principalInvestigator}</p>
                        <p>Assigné à: <strong>{protocol.assignedTo}</strong></p>
                        {protocol.assignedAt && (
                          <p>Le: {new Date(protocol.assignedAt).toLocaleDateString('fr-FR')}</p>
                        )}
                      </div>
                    </div>
                    <span style={{
...
export default DistributeProtocols;
